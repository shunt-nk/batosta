<?php
// avatar_customize.php 既存機能温存 + 正しいスロット整合 + パス仕様 + 安全装備更新
...
/* ---------------- スロット名の相互変換（UI↔DB） ----------------
 * UIは weapon/shield/head/outfit/boots を使用
 * DB保存も基本は同名を使用（ここが重要！）
 * 読み出し時のみ古い表記 armor/body/legs を UI 名に吸収
 */
function ui_to_db_slot(string $ui): string {
  static $map = [
    'weapon'=>'weapon', 'shield'=>'shield', 'head'=>'head',
    'outfit'=>'outfit', 'boots'=>'boots'
  ];
  return $map[$ui] ?? $ui;
}
function db_to_ui_slot(string $db): string {
  static $map = [
    'weapon'=>'weapon', 'shield'=>'shield', 'head'=>'head',
    'outfit'=>'outfit', 'armor'=>'outfit', // 旧表記を吸収
    'boots'=>'boots',  'legs'=>'boots',    // 旧表記を吸収
    'body'=>'outfit',                     // 旧表記を吸収
  ];
  return $map[$db] ?? $db;
}
...
function build_path(string $base, string $slotUi, ?string $file): string {
  $file = norm_file($file);
  if ($file === '') return '';
  if (preg_match('~^(assets/|https?://)~i', $file)) return equip_img_src($file);
  if (strpos($file, '/') !== false) return equip_img_src($base.'/'.$file);
...
function fetch_empty_icon_map(PDO $pdo, array $uiSlots): array {
  ...
  try {
    ...
    foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
      $ui = db_to_ui_slot((string)$r['slot']); // DB側が armor 等でも吸収
...
/* ---------- POST: 装備 or 解除（DELETE→INSERT） ---------- */
if ($_SERVER['REQUEST_METHOD']==='POST') {
  $action = $_POST['equip_action'] ?? '';
  $slotUiP  = $_POST['slot'] ?? '';
  if (in_array($slotUiP, $SLOTS_UI, true)) {
    $slotDb = ui_to_db_slot($slotUiP); // ここで body/legs に変換しないのが重要
    if ($action==='equip') {
...
} catch (\Throwable $e) { /* 権限無しでもOK（両方false） */ }
...
<div class="container">
  ...
  <main class="customize">
    ...
    <section class="left">
      ...
      <!-- アバター本体（装備は functions.php の renderAvatarFull が avatars で重ねる） -->
        ...
        <?php if ($selected): ?>
          <div class="detail-thumb">
            <?php
              ...
              $detailIconSrc = ui_icon_src($selSlot, $selected, $emptyIconMap); // 右のサムネは icons
            ?>
